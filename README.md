...
- 写作助手：第一步行动、第二步证据、第三步未来身份都可以点按钮优化；写“我是...”卡住时，也可以点词库，不需要调用 AI API。
- 证据历史：界面只显示最近 3 天的“我是...”句子，而且每条记录只显示一句，避免历史区变得太长；完整记录仍会保存在本地/云端。
- 历史管理：可以删除不需要保留的历史；登录云端时会尝试同步删除对应记录。
...
前端本身没有直接调用 LLM API，也不会把 AI key 放进浏览器。

如果你部署了 `polish-identity` Supabase Edge Function，并设置了 `OPENROUTER_API_KEY`，app 会在你点击 `优化并展开我这句` 时通过 Edge Function 调 OpenRouter。
...
- GitHub Pages 静态部署。
- Supabase 登录和资料保存。
- Supabase Edge Function 里的 OpenRouter AI 句子优化。
- Horizon feed / GitHub / DEV / Wikipedia 等公开资料读取。
- 用户导入的 `.md` / `.txt` 文件清理。

“有钱人的世界”里很多句子是写在代码里的模板；online key points 是从公开来源抓取并显示。只有“优化并展开我这句”会在配置完成后调用 OpenRouter。
...
- OpenRouter：`openrouter/free` 有免费模型和低额度限制；如果改用付费模型或超过免费条件，会消耗 OpenRouter credits。
...
这个项目支持用 Supabase Edge Function 调 OpenRouter 来优化“我是...”句子。

安全原则：

- 不要把 `OPENROUTER_API_KEY` 放进 `config.js`。
...
```powershell
npx supabase secrets set OPENROUTER_API_KEY=你的-openrouter-api-key
```
